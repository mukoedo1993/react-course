...
function HomeGuest() {
  ...
  const initialState = {
    username: {
      ...
      isUnique: false,
      checkCount: 0, //If that changes, we could send Axio request to see if it is unique or not.
    },
    email: {
      value: "",
      hasErrors: false,
  ...
  function ourReducer(draft, action) {
    switch (action.type) {
      case "usernameImmediately": //to make sure our username does not exceed 30 chars and is alphanumeric:
    ...
    if (state.username.checkCount) {
      ...
      async function fetchResults() {
        try {
          ...
          dispatch({ type: "usernameUniqueResults", value: response.data }) //response.data here is true or false.
        } catch (e) {
          console.log("There was a problem or the request was cancelled within HomeGuest.js file for state.username.checkCount.")
          console.log(e)
        }
      }
      fetchResults()
    ...
    if (state.email.checkCount) {
      ...
      async function fetchResults() {
        try {
          ...
          dispatch({ type: "emailUniqueResults", value: response.data }) //response.data here is true or false.
        } catch (e) {
          console.log("There was a problem or the request was cancelled within HomeGuest.js file for state.email.checkCount.")
          console.log(e)
        }
      }
      fetchResults()
  ...
  function handleSubmit(e) {
    ...
    dispatch({ type: "usernameAfterDelay", value: state.username.value, noRequest: true }) //We dp not need to check if username is unique here.
    dispatch({ type: "emailImmediately", value: state.email.value })
    dispatch({ type: "emailAfterDelay", value: state.email.value, noRequest: true }) //We do not need to check if email is unique here.
    ...
    <Page wide={true} title="Welcome">
      <div className="row align-items-center">
        <div className="col-lg-7 py-3 py-md-5">
          ...
          <p className="lead text-muted">Are you sick of short tweets and impersonal &ldquo;shared&rdquo; posts that are reminiscent of the late 90&rsquo;s email htmlForwards? We believe getting back to actually writing is the key to enjoying the internet again.</p>
        </div>
